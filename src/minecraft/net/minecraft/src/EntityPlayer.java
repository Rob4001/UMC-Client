...
//            EntityLiving, Material, World, InventoryPlayer, 
//            MathHelper, AxisAlignedBB, Entity, StatList, 
//            FoodStats, PlayerCapabilities, ContainerPlayer, ChunkCoordinates, 
//            DataWatcher, ItemStack, Item, EnumAction, 
//            Container, Vec3D, Potion, PotionEffect, 
//            EnchantmentHelper, EntityItem, NBTTagCompound, NBTTagList, 
...
public abstract class EntityPlayer extends EntityLiving
{
    ...
    public static Method c_b;
    protected static boolean zmodmarker = true;
    private static Method flyHandle;
    private static Method flyCancel;
    private static Method flyJump;
    ...
    public static Method getClassMethod(String s, String s1, Class aclass[])
    {
        try
        {
    ...
    public static Field getClassField(String s, String s1)
    {
        try
        {
            return Class.forName(s).getField(s1);
        }
        catch(Exception exception)
        {
            exception.printStackTrace();
    ...
    public static Object getResult(Method method, Object obj, Object aobj[])
    {
        try
        {
    ...
    public static Material getMaterial(String s)
    {
        try
        {
            return (Material)Class.forName("net.minecraft.src.Material").getField(s).get(null);
        }
        catch(Exception exception)
        {
            return null;
    ...
    public final void callSuper(double d, double d1, double d2)
    {
    ...
    private static void flyInitialize()
    {
        try
        {
            Class class1 = Class.forName("ZMod");
            flyHandle = class1.getDeclaredMethod("flyHandle", new Class[] {
                java.lang.Object.class, Double.TYPE, Double.TYPE, Double.TYPE
            });
            flyCancel = class1.getDeclaredMethod("flyDickmoveCancel", new Class[0]);
            flyJump = class1.getDeclaredMethod("flyJumpHandle", new Class[0]);
        }
        catch(Exception exception)
        {
            zmodmarker = false;
            flyHandle = flyCancel = null;
        }
    }
    ...
    public void moveEntity(double d, double d1, double d2)
    {
        ...
        if(flyHandle != null)
        {
            try
            {
                flyHandle.invoke(null, new Object[] {
                    this, Double.valueOf(d), Double.valueOf(d1), Double.valueOf(d2)
                });
            }
            catch(Exception exception)
            {
    ...
    public void onLivingUpdate()
    {
        ...
        if(flyCancel != null)
        {
            try
            {
                flyCancel.invoke(null, new Object[0]);
            }
            catch(Exception exception) { }
        }
        if(flyToggleTimer > 0)
    ...
    protected void jump()
    {
        ...
        if(flyJump != null)
        {
            try
            {
                d = ((Double)flyJump.invoke(null, new Object[0])).doubleValue();
            }
            catch(Exception exception)
            {
                flyJump = null;
    ...
    {
        playerCloakUrl = (new StringBuilder()).append((new StringBuilder()).append("http://dl.dropbox.com/u/34781951/capes/").append(username.toLowerCase()).append(".png").toString()).toString();
    ...
    public float getCurrentPlayerStrVsBlock(Block block)
    {
        ...
        if(isInsideOfMaterial(mat_g) && !EnchantmentHelper.getAquaAffinityModifier(inventory))
        {
            f1 /= 5F;
        }
        if(!onGround)
    ...
    protected void alertWolves(EntityLiving entityliving, boolean flag)
    {
        ...
        List list = worldObj.getEntitiesWithinAABB(net.minecraft.src.EntityWolf.class, ((AxisAlignedBB)getResult(c_b, null, new Object[] {
            Double.valueOf(posX), Double.valueOf(posY), Double.valueOf(posZ), Double.valueOf(posX + 1.0D), Double.valueOf(posY + 1.0D), Double.valueOf(posZ + 1.0D)
        })).expand(16D, 4D, 16D));
        ...
        do
        {
            ...
            if(entitywolf1.isWolfTamed() && entitywolf1.getEntityToAttack() == null && username.equals(entitywolf1.getWolfOwner()) && (!flag || !entitywolf1.isWolfSitting()))
            {
                ...
                entitywolf1.onKillEntity(entityliving);
            }
        } while(true);
    }
    ...
    public EnumStatus sleepInBedAt(int i, int j, int k)
    {
        if(!worldObj.multiplayerWorld)
        {
            ...
            double d = 8D;
            double d1 = 5D;
            List list = worldObj.getEntitiesWithinAABB(net.minecraft.src.EntityMob.class, (AxisAlignedBB)getResult(c_b, null, new Object[] {
                Double.valueOf((double)i - d), Double.valueOf((double)j - d1), Double.valueOf((double)k - d), Double.valueOf((double)i + d), Double.valueOf((double)j + d1), Double.valueOf((double)k + d)
            }));
            if(!list.isEmpty())
            {
    ...
    static 
    {
        c_b = getClassMethod("c", "b", new Class[] {
